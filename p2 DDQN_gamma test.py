...
"""
"""
THIS PYTHON FILE FOCUS ON SENSITIVITY TEST OF DISCOUNT FACTOR GAMMA
"""
...
if __name__ == '__main__':
    ...
    #PLOT TRAIN RESULT 
    x = [i + 1 for i in range(train_episode)]
    ...
    plt.plot(x, reward_final, color ='b', markersize = 3)
    plt.plot(x, reward_SMA1, color = 'r', linewidth = 2)
    plt.title('reward at each episode, gamma = 0.99')
    plt.xlabel('episode')
    plt.ylabel('reward')
    plt.ylim(-500, 500)
    plt.grid(True)
    plt.savefig('p2.gamma0.99.png')
    # plt.show()
    plt.clf()
            
    # GAMMA = 0.90
    agent = Agent(gamma = 0.90, epsilon = 1.00, alpha = 0.0003, state_dim=[8],
                  action_dim = 4, batch_size = 64, eps_min= 0.01, eps_decay=0.001)
    reward_final = []
    ...
    #PLOT TRAIN RESULT 
    # x = [i + 1 for i in range(train_episode)]
    reward_SMA2 = [np.mean(reward_final[max(0, i -100):i+1]) for i in range(train_episode)]
    # plt.plot(x, reward_final, 'o', color ='b', markersize = 3)
    plt.plot(x, reward_final, color ='b', markersize = 3)
    plt.plot(x, reward_SMA2, color = 'r', linewidth = 2)
    plt.title('reward at each episode, gamma 0.90')
    plt.xlabel('episode')
    plt.ylabel('reward')
    plt.ylim(-500, 500)
    plt.grid(True)
    plt.savefig('p2.gamma0.90.png')
    # plt.show()
    plt.clf()

    # GAMMA = 0.75
    agent = Agent(gamma = 0.75, epsilon = 1.00, alpha = 0.0003, state_dim=[8],
                  action_dim = 4, batch_size = 64, eps_min= 0.01, eps_decay=0.001)
    reward_final = []
    ...
    #PLOT TRAIN RESULT 
    # x = [i + 1 for i in range(train_episode)]
    reward_SMA3 = [np.mean(reward_final[max(0, i -100):i+1]) for i in range(train_episode)]
    # plt.plot(x, reward_final, 'o', color ='b', markersize = 3)
    plt.plot(x, reward_final, color ='b', markersize = 3)
    plt.plot(x, reward_SMA3, color = 'r', linewidth = 2)
    plt.title('reward at each episode, gamma = 0.75')
    plt.xlabel('episode')
    plt.ylabel('reward')
    plt.ylim(-500, 500)
    plt.grid(True)
    plt.savefig('p2.gamma0.75.png')
    # plt.show()
    plt.clf()

    # GAMMA = 0.50
    agent = Agent(gamma = 0.50, epsilon = 1.00, alpha = 0.0003, state_dim=[8],
                  action_dim = 4, batch_size = 64, eps_min= 0.01, eps_decay=0.001)
    reward_final = []
    ...
    #PLOT TRAIN RESULT 
    # x = [i + 1 for i in range(train_episode)]
    reward_SMA4 = [np.mean(reward_final[max(0, i -100):i+1]) for i in range(train_episode)]
    # plt.plot(x, reward_final, 'o', color ='b', markersize = 3)
    plt.plot(x, reward_final, color ='b', markersize = 3)
    plt.plot(x, reward_SMA4, color = 'r', linewidth = 2)
    plt.title('reward at each episode, gamma = 0.50')
    plt.xlabel('episode')
    plt.ylabel('reward')
    plt.ylim(-500, 500)
    plt.grid(True)
    plt.savefig('p2.gamma0.50.png')
    # plt.show()
    plt.clf()

    # PLOT SUMMARY

    plt.plot(x, reward_SMA1, label = 'gamma = 0.99', color = 'blue')
    plt.plot(x, reward_SMA2, label = 'gamma = 0.90', color = 'red')
    plt.plot(x, reward_SMA3, label = 'gamma = 0.75', color = 'green')
    plt.plot(x, reward_SMA4, label = 'gamma = 0.50', color = 'orange')
    plt.title('reward on learner with different gamma')
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.legend()
    plt.savefig('p2.gamma.summary.png')
    # plt.show()
    plt.clf()